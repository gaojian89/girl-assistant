 ...
 *******************************************************************************/
package com.girlassistant.view.pulltorefresh;

import com.girlassistant.activity.R;
import com.girlassistant.utils.MLog;
import com.girlassistant.view.pulltorefresh.internal.FlipLoadingLayout;
import com.girlassistant.view.pulltorefresh.internal.LoadingLayout;
import com.girlassistant.view.pulltorefresh.internal.RotateLoadingLayout;
import com.girlassistant.view.pulltorefresh.internal.Utils;
import com.girlassistant.view.pulltorefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	boolean isHideAddMoreView = false;

	public boolean isHideAddMoreView() {
		return isHideAddMoreView;
	}

	public void setHideAddMoreView(boolean isHideAddMoreView) {
		this.isHideAddMoreView = isHideAddMoreView;
	}

	Context context;

	private boolean isAutoAddMore = false;

	public boolean isAutoAddMore() {
		return isAutoAddMore;
	}

	public void setAutoAddMore(boolean isAutoAddMore) {
		this.isAutoAddMore = isAutoAddMore;
	}

	private Mode customMode = Mode.getDefault();

	public Mode getCustomMode() {
		return customMode;
	}

	public void setCustomMode(Mode customMode) {
		this.customMode = customMode;
	}

	int myfootViewState = 0;

	public int getMyfootViewState() {
		return myfootViewState;
	}

	public void setMyfootViewState(int myfootViewState) {
		this.myfootViewState = myfootViewState;
	}

	final int myfootViewState_add_more = 1;
	final int myfootViewState_loading = 2;
	final int myfootviewstate_invisible = 3;
	final int myfootViewState_gone = 4;

	protected LinearLayout mLvFooterLoadingLayout;
	protected LinearLayout mLvFooterLoadingMore;
	protected LinearLayout mLvFooterLoading;
	protected TextView load_more;

	public void changeMyfootViewState() {

		switch (myfootViewState) {
		case myfootViewState_add_more:
			MLog.s("-----------显示");
			mLvFooterLoadingLayout.setVisibility(View.VISIBLE);
			mLvFooterLoadingMore.setVisibility(View.VISIBLE);
			mLvFooterLoading.setVisibility(View.GONE);
			break;
		case myfootViewState_loading:
			MLog.s("-----------加载中....");
			mLvFooterLoadingLayout.setVisibility(View.VISIBLE);
			mLvFooterLoadingMore.setVisibility(View.GONE);
			mLvFooterLoading.setVisibility(View.VISIBLE);
			break;
		case myfootviewstate_invisible:
			MLog.s("-----------隐藏View.INVISIBLE");
			mLvFooterLoadingLayout.setVisibility(View.INVISIBLE);
			mLvFooterLoadingMore.setVisibility(View.INVISIBLE);
			mLvFooterLoading.setVisibility(View.GONE);
			break;
		case myfootViewState_gone:
			MLog.s("-----------隐藏View.GONE");
			mLvFooterLoadingLayout.setVisibility(View.GONE);
			mLvFooterLoadingMore.setVisibility(View.GONE);
			mLvFooterLoading.setVisibility(View.GONE);
			break;	
	...
	// public void changeAddMoreMode(MotionEvent event) {
	//
	// final int action = event.getAction();
	// if (action == MotionEvent.ACTION_DOWN /* ||(!isPullToRefreshEnabled())
	// */) {
	// // if (isReadyForPullEnd()) {
	// if (isAutoAddMore()) {
	// setModeInternal(getCustomMode());
	// setMyfootViewState(myfootViewState_none);
	// changeMyfootViewState();
	//
	// } else {
	// if (getMyfootViewState() != myfootViewState_loading
	// && !isRefreshing()) {
	//
	// if (customMode == Mode.BOTH) {
	// System.out.println("Mode.BOTH");
	// setModeInternal(Mode.PULL_FROM_START);
	// if (!isHideAddMoreView()) {
	// setMyfootViewState(myfootViewState_add_more);
	// changeMyfootViewState();
	// }
	//
	// } else if (customMode == Mode.PULL_FROM_END) {
	// System.out.println("Mode.PULL_FROM_END");
	// setModeInternal(Mode.DISABLED);
	// if (!isHideAddMoreView()) {
	// setMyfootViewState(myfootViewState_add_more);
	// changeMyfootViewState();
	// }
	...
	public void changeAddMoreMode(MotionEvent event) {
		...
		if (getMyfootViewState() == myfootViewState_loading || isRefreshing()) {
			return;
		}
		final int action = event.getAction();
		if (action == MotionEvent.ACTION_DOWN) {
			if (isAutoAddMore()&&getMyfootViewState() !=myfootViewState_loading) {
				if(!isHideAddMoreView()){
					setMyfootViewState(myfootviewstate_invisible);
					changeMyfootViewState();	
				}
	...
	public void showAddMoreMode(){
		

		if (!isHideAddMoreView()&&!isAutoAddMore&&isDataAdequacy()) {
			setMyfootViewState(myfootViewState_add_more);
			changeMyfootViewState();
		}
	}
	
	protected abstract boolean isDataAdequacy();
	...
	@Override
	public final Mode getModeInternal() {
		return mMode;
	}
	...
	@Override
	public final void setModeInternal(Mode mode) {
...
//			showAddMoreMode();
	...
	public final void setPullToRefreshEnabled(boolean enable) {
		setModeInternal(enable ? Mode.getDefault() : Mode.DISABLED);
	}
	...
	protected final void onRestoreInstanceState(Parcelable state) {
		if (state instanceof Bundle) {
			...
			setModeInternal(Mode.mapIntToValue(bundle.getInt(STATE_MODE, 0)));
