package com.girlassistant.view;
...
import com.girlassistant.view.ScrollOverListView.OnScrollOverListener;
...
public class PullDownView extends LinearLayout implements OnScrollOverListener, OnScrollListener {
	...
	public interface HeadPullDownSpecificDistanceListener {
		void onReachTheDistance(int distancePix);
	}

	private HeadPullDownSpecificDistanceListener specificDistanceListener;
...
//	private Button btnCheckWifi;
	...
	public static final int STATE_LOADING_MORE = 2; // 加载更多中

	public static final int STATE_DRAGING = 4;// 拖动中

	public static final int STATE_MOTION_DOWN = 8; // 按下

	public static final int STATE_NO_MORE_TO_LOAD = 16;
	...
	public void setSpecifcDistanceListener(HeadPullDownSpecificDistanceListener l) {
		specificDistanceListener = l;
	}
	...
	private void initHeaderViewAndFooterViewAndListView(final Context context) {
		...
		mHeaderView = LayoutInflater.from(context).inflate(R.layout.refresh_list_header, null);

		mHeaderViewParams = new LayoutParams(android.view.ViewGroup.LayoutParams.MATCH_PARENT, android.view.ViewGroup.LayoutParams.WRAP_CONTENT);

		mHeaderIncremental = mDefaultHeaderViewHeight;

		addView(mHeaderView, 0, mHeaderViewParams);

		// mDefaultHeaderViewHeight =
		// getResources().getDimensionPixelSize(R.dimen.pulldown_headerview_height);
		//
		// mMoveDeviation =
		// getResources().getDimensionPixelSize(R.dimen.pulldown_move_deviation);
		mDefaultHeaderViewHeight = 50;
		mMoveDeviation = 50;
		mHeaderTextView = (TextView) mHeaderView.findViewById(R.id.refresh_list_header_text);

		mHeaderArrowView = (ImageView) mHeaderView.findViewById(R.id.refresh_list_header_pull_down);

		mHeaderViewDateView = (TextView) mHeaderView.findViewById(R.id.refresh_list_header_last_update);

		mHeaderLoadingView = mHeaderView.findViewById(R.id.refresh_list_header_progressbar);
		...
		mFooterView = (FrameLayout) LayoutInflater.from(mContext).inflate(R.layout.pulldown_footer, null);

		mFooterView.setVisibility(View.GONE);

//		mEmptyContainer = (RelativeLayout) mFooterView.findViewById(R.id.pulldown_empty_container);

//		btnCheckWifi = (Button) mFooterView.findViewById(R.id.midlayout_btn_loadnews_failed);
//		btnCheckWifi.setOnClickListener(new OnClickListener() {
//
//			@Override
//			public void onClick(View v) {
//				context.startActivity(new Intent(context, NetworkSettingsAct.class));
//			}
//		});

		mHintContainer = (LinearLayout) mFooterView.findViewById(R.id.pulldown_hint_container);
		...
		addView(mListView, android.view.ViewGroup.LayoutParams.MATCH_PARENT, android.view.ViewGroup.LayoutParams.MATCH_PARENT);
		// set ListView animation

		/*
		 * 
		 * AnimationSet set = new AnimationSet(true); Animation animation = new
...
//	public RelativeLayout getEmptyContainer() {
...
//			mEmptyContainer.setVisibility(View.GONE);
	...
	private void setHeaderHeight(final int height) {
		...
		if (specificDistanceListener != null) {
			specificDistanceListener.onReachTheDistance(height);
		}
	...
	public boolean onListViewBottomAndPullUp(MotionEvent event, int delta) {
		...
		|| !mIsDidLoad || !mEnableAutoFetchMore || mIsNoMoreData)
			return false;
	...
	private void setListViewSelection() {
		if (mListView.getAdapter() != null) {
			int position = mStartIndex;
			int[] childLocation = new int[2];
			View child = mListView.getAdapter().getView(position, null, mListView);
			child.getLocationInWindow(childLocation);
			int halfHeight = child.getHeight() / 2;
			int[] listLocation = new int[2];
			mListView.getLocationInWindow(listLocation);
			if (listLocation[1] - childLocation[1] > halfHeight) {
				position++;
			}
			if (position < mTotalCount) {
				mUIHandler.removeCallbacks(mSetSelectionRunnable);
				mSetSelectionRunnable.setPostion(position);
				mUIHandler.postDelayed(mSetSelectionRunnable, 200);
			}
		}
	}

	private SetSelectionRunnable mSetSelectionRunnable = new SetSelectionRunnable();
	...
	public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
		...
		if (firstVisibleItem + visibleItemCount == totalItemCount) {
			if ((state & STATE_LOADING_MORE) == STATE_LOADING_MORE || !mIsDidLoad || !mEnableAutoFetchMore || mIsNoMoreData)
				return;
		 ...
		 * boolean hasItem = localListView.getCount() > 0; try {
		 * 
		 * if (overScrollModeField == null) return;
		 * 
		 * final Integer mode = (Integer)
		 * overScrollModeField.get(localListView); if (firstVisibleItem <= 0 &&
		 * hasItem) {
		 * 
	...
	public boolean ismIsPullUpDone() {
		return mIsPullUpDone;
	}
